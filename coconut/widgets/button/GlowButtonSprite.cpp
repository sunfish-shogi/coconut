//
//  GlowButtonSprite.cpp
//  coconut
//
//  Created by Kubo Ryosuke on 2014/01/27.
//
//

#include "GlowButtonSprite.h"

USING_NS_CC;

#define TEXTURE_KEY_GLOW									"/___glowImageData"

namespace coconut {
	
	namespace {
		const unsigned char glowImageData[] = {
			
		};
	};
	
	bool GlowButtonSprite::init(Node* sprite, bool front, const Color3B& color, float glowStrength) {
		if (!ButtonSprite::init()) {
			return false;
		}
		
		CCASSERT(sprite != nullptr, "sprite must not be null.");
		
		_sprite = sprite;
		addChild(_sprite, 0);
		
		Texture2D* texture = Director::getInstance()->getTextureCache()->getTextureForKey(TEXTURE_KEY_GLOW);
		if (texture == nullptr) {
			Image* glowImage = new Image();
			glowImage->initWithImageData(glowImageData, sizeof(glowImageData));
			texture = Director::getInstance()->getTextureCache()->addImage(glowImage, TEXTURE_KEY_GLOW);
			delete glowImage;
		}
		_glow = Sprite::createWithTexture(texture);
		_glow->setColor(color);
		float width = _sprite->getContentSize().width * _sprite->getScaleX();
		float height = _sprite->getContentSize().height * _sprite->getScaleY();
		_glow->setScaleX((width * 1.3f + height * 0.2) * glowStrength / _glow->getContentSize().width);
		_glow->setScaleY((height * 1.3f + width * 0.2) * glowStrength / _glow->getContentSize().height);
		_glow->setOpacity(front ? 128 : 255);
		addChild(_glow, (front ? 1 : -1));
		
		reset();
		
		return true;
	}
	
	bool GlowButtonSprite::isInside(Touch* touch) {
		if (_sprite != nullptr) {
			const Point& pos = _sprite->convertToNodeSpace(Director::getInstance()->convertToUI(touch->getLocationInView()));
			const Point& targetPos = _sprite->getPosition();
			const Point& anc = _sprite->getPosition();
			const Size& size = _sprite->getContentSize();
			const Rect rect(targetPos.x - size.width * anc.x,
											targetPos.y - size.height * anc.y,
											size.width, size.height);
			return rect.containsPoint(pos);
		}
		return false;
	}
	
	void GlowButtonSprite::reset() {
		detouch();
	}
		
	void GlowButtonSprite::touch() {
		_glow->setVisible(true);
	}
	
	void GlowButtonSprite::detouch() {
		_glow->setVisible(false);
	}
	
	GlowButtonSprite* GlowButtonSprite::create(Node* sprite, bool front, const Color3B& color, float glowStrength) {
		GlowButtonSprite* instance = new GlowButtonSprite();
		if (instance != nullptr && instance->init(sprite, front, color, glowStrength)) {
			instance->autorelease();
			return instance;
		}
		CC_SAFE_DELETE(instance);
		return nullptr;
	}
	
	GlowButtonSprite* GlowButtonSprite::createWithSpriteFileName(const char* sprite, bool front, const Color3B& color, float glowStrength) {
		GlowButtonSprite* instance = new GlowButtonSprite();
		if (instance != nullptr && instance->init(Sprite::create(sprite), front, color, glowStrength)) {
			instance->autorelease();
			return instance;
		}
		CC_SAFE_DELETE(instance);
		return nullptr;
	}
	
	GlowButtonSprite* GlowButtonSprite::createWithSpriteFrameName(const char* sprite, bool front, const Color3B& color, float glowStrength) {
		GlowButtonSprite* instance = new GlowButtonSprite();
		if (instance != nullptr && instance->init(Sprite::createWithSpriteFrameName(sprite), front, color, glowStrength)) {
			instance->autorelease();
			return instance;
		}
		CC_SAFE_DELETE(instance);
		return nullptr;
	}
	
	GlowButtonSprite* GlowButtonSprite::createWithSpriteFrame(SpriteFrame* sprite, bool front, const Color3B& color, float glowStrength) {
		GlowButtonSprite* instance = new GlowButtonSprite();
		if (instance != nullptr && instance->init(Sprite::createWithSpriteFrame(sprite), front, color, glowStrength)) {
			instance->autorelease();
			return instance;
		}
		CC_SAFE_DELETE(instance);
		return nullptr;
	}
	
};
